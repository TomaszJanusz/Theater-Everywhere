...
Theater Mode Everywhere è pensato per chiunque voglia guardare video sul web senza l'ingombro di intestazioni, barre laterali, commenti o raccomandazioni. Che tu stia guardando tutorial, streaming o brevi clip, puoi trasformare qualsiasi pagina video in un accogliente cinema con un solo tasto.
...
COSA È SUPPORTATO
- Video HTML5 universale: Funziona con gli elementi standard HTML5 `<video>` su tutto il web, inclusi i riproduttori inseriti nel Shadow DOM.
- Riproduttori incorporati (iFrame): Rileva ed espande i video incorporati all'interno di frame, come le incorporazioni di YouTube o Vimeo.
- Sottotitoli non standard: I sottotitoli HTML5 standard (tag `<track>`) sono pienamente supportati. Se un sito web mostra i sottotitoli tramite sovrapposizioni personalizzate, JavaScript proprietario o elementi esterni, tali sottotitoli potrebbero non essere visualizzati all'interno dei controlli personalizzati.
- Servizi protetti da DRM: Alcune piattaforme di streaming utilizzano tecnologie DRM che limitano la manipolazione del DOM video, impedendo all'estensione di ridimensionare correttamente il video.

COME INIZIARE
1. Apri una pagina contenente un video (ad es. piattaforme video, siti di notizie o blog).
2. Premi `T` sulla tastiera. Assicurati che il cursore non sia posizionato in una barra di ricerca o in un campo per i commenti.
3. Utilizza i controlli personalizzati in basso, le frecce `Sinistra` / `Destra` per andare avanti/indietro e `<` / `>` per avanzare fotogramma per fotogramma.
4. Premi `Escape` o `T` per uscire dalla modalità cinema.

## Suggested keywords
video, modalità cinema, modalità teatro, video html5, schermo intero, picture in picture, amplificazione volume, scorciatoie da tastiera, youtube, vimeo
